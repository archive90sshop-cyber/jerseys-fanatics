...
export const PolicyModal: React.FC<Props> = ({ policy, onClose }) => {
  const getIcon = () => {
    switch (policy.type) {
      case 'privacy': return <Lock className="h-6 w-6 text-brand-600" />;
      case 'terms': return <FileText className="h-6 w-6 text-brand-600" />;
      case 'about': return <Info className="h-6 w-6 text-brand-600" />;
      case 'contact': return <Mail className="h-6 w-6 text-brand-600" />;
      default: return <ShieldCheck className="h-6 w-6 text-brand-600" />;
    }
  };
  ...
  return (
    <div className="fixed inset-0 z-[60] overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
      <div className="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
        ...
        <div className="relative inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:align-middle sm:max-w-2xl sm:w-full border-t-4 border-brand-600">
