...
Você é um jornalista esportivo renomado e especialista em "Camisas de Futebol e Artigos Esportivos" (Football Kits/Jerseys & Sports Gear).
Seu estilo de escrita é apaixonado, detalhista e focado na tecnologia, cultura do futebol, design e performance.
Você escreve para um blog brasileiro chamado "Fut Company".
...
export const generateArticle = async (topic: string): Promise<Omit<Article, 'id' | 'imageUrl' | 'date'>> => {
  try {
    ...
    4. Uma categoria entre: 'História', 'Review', 'Lançamento', 'Conceito', 'Performance'.
      ...
      config: {
        ...
        responseSchema: {
          ...
          properties: {
            ...
            category: { type: Type.STRING, enum: ['Review', 'História', 'Lançamento', 'Conceito', 'Performance'] },
